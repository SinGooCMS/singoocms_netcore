...
{
    public class MultipleHtml : FieldControl
    {
        public MultipleHtml(ICMSContext _context, IAttachmentRepository _attachmentRepository, IDictItemRepository _dictItemRepository)
        {
        ...
        public override FieldType FieldType => FieldType.MultipleHtml;

        public override async Task<string> Render()
        {
            string path = $"/views/platform/{context.SiteConfig.Theme}/FieldControls/MultipleHtmlType.cshtml";
            ...
            {
                string width = this.Settings.ControlWidth <= 100 ? this.Settings.ControlWidth + "%" : "";
                builder.AppendFormat(" singoo-editor-param={0}|{1}", width, this.Settings.ControlHeight);
            }
            ...
            if (!this.EnableNull)
                builder.AppendFormat(" required=\"required\"");
            ...
            return txt.Replace("${id}", this.FieldName);
        }
    }
}
